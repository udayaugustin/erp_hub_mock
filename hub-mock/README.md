# ABC Trading Central E-Invoicing Hub · guided walkthrough

A navigable prototype of the **central e-invoicing hub** proposed for **The ABC Group
...
Every member of the ABC Group VAT Group invoices under the *same* group TRN
`OM1200094685`. That is one filing identity — but twelve separate legal entities, spread across
four different ERPs:

- **Enterprise ERP** — the lead entity, ABC Trading LLC, plus three more (XYZ
  Industries, BCD Services, DEF Properties).
- **High-Volume Billing** — six automotive/retail entities, including the volume giant **PQR Retail
  Group LLC** (~66k simplified B2C invoices a month).
- **Finance Suite** — GHI Building Solutions LLC.
- **Accounting System** — JKL Logistics LLC.
...
| 2 | ERP — Counter & Service Sales (High-Volume Billing) | The group's second invoice origin — PQR Retail's high-volume B2C counter and service sales, collected by High-Volume Billing and reported to the OTA in batches through the same Hub |
...
**`ABC-SINV-2026-00841`** — issued by the lead entity **ABC Trading LLC** (CR `1008431`,
...
the entity's own system: **Enterprise ERP** on screens 1 and 18, and **High-Volume Billing** on screen 2,
the group's second invoice origin (PQR Retail's B2C counter sales). The point of all three
...
- **Batch B2C.** PQR Retail's ~66k/month simplified invoices are reported to the OTA in
  **batches from High-Volume Billing**, not cleared live at the point of sale. This is flagged as an
  *Assumption · to confirm with the group* on the relevant screen.
...
`assets/js/data.js` holds an illustrative dataset for the **ABC Group VAT Group** —
...
The twelve entities are **real ABC Trading companies** — see `ZUBAIR-REAL-ENTITIES.md` for
...
> Counterparties, people and email addresses (`@abcgroup.example`) are fictional on purpose.
...
- Screens 9 and 10 follow the same invoice through consecutive stages. Screens 1, 17 and 18 all
  show `ABC-SINV-2026-00841`, so the walkthrough closes a loop on one document.
