...
with sync_playwright() as pw:
    ...
    for bad in ['Muscat Steel', 'Gulf Logistics', 'Salalah Foods', 'Duqm Cement',
                'Nizwa', 'Barka Retail', 'MSI-', 'GLC-', 'SFP-', 'DQC-', 'NPC-']:
        if bad in txt:
            fail(f'portal/dashboard.html  isolation breach → "{bad}"')
    if 'Towell Auto Centre' not in txt:
    ...
    for rel in ['erp/invoices.html', 'portal/dashboard.html', 'erp/sync.html']:
        load(rel)
        if 'TAC-SINV-2026-04471' not in pg.inner_text('body'):
            fail(f'{rel}  tracked invoice TAC-SINV-2026-04471 missing')
