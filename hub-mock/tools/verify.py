...
# survive the rebrand to the generic ABC Group. Scanned in visible HTML and in
...
with sync_playwright() as pw:
    ...
    # The single-entity portal shows ABC Trading LLC (ABC) only. No OTHER group
    ...
    for bad in ['PQR Retail', 'RST Retail', 'JKL Logistics', 'YZA Heavy',
                'STU Motors', 'MNO Automotive', 'XYZ Industries', 'GHI Building',
                'PQR-', 'RST-', 'JKL-', 'YZA-', 'STU-', 'MNO-', 'XYZ-', 'BCD-', 'GHI-', 'VWX-', 'DEF-']:
        if bad in txt:
            fail(f'portal/dashboard.html  isolation breach → "{bad}"')
    if 'ABC Trading' not in txt:
    ...
    for rel in ['erp/invoices.html', 'portal/dashboard.html', 'erp/sync.html']:
        load(rel)
        if 'ABC-SINV-2026-00841' not in pg.inner_text('body'):
            fail(f'{rel}  tracked invoice ABC-SINV-2026-00841 missing')
