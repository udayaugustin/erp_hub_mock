# the group hub mock — gap register against the proposal
Re-pointed 18 Aug 2026 for The ABC Group VAT Group presentation. This build is a clone of
the proven multi-entity hub; the rules below are inherited from that build's audit and re-targeted
to the twelve-entity, single-VAT-group the group story. `tools/verify.py` enforces them as a lint
...
| C4 | **Batch B2C.** PQR Retail's ~66k/month simplified invoices are reported in **batches from High-Volume Billing**, not live at point of sale — carry the *"Assumption · to confirm with the group"* card. | Load-bearing demo assumption. See `ZUBAIR-QUESTIONS.md` Q2. |
...
buyer with an invented VAT number. The only real names in the dataset are the twelve the group
