...
# Clone-drift: strings from the wj-mock and zubair-mock bases that must never
# survive the rebrand to the generic OIB Group. Scanned in visible HTML and in
# full JS. All tokens are case-sensitive substrings, so the uppercase 3-letter
# entity codes below cannot false-match lowercase words (OCS vs "docs" etc.).
...
with sync_playwright() as pw:
    ...
    # The single-entity portal shows Oman Investment Bank SAOC (ABC) only. No OTHER group
    ...
    for bad in ['OIB Digital Payments', 'OIB Merchant Settlement', 'OIB Custody & Fund Services', 'YZA Heavy',
                'OIB Corporate Cards', 'OIB Salalah Payments', 'OIB Asset Management', 'OIB Trade Finance',
                'PQR-', 'RST-', 'JKL-', 'YZA-', 'STU-', 'MNO-', 'XYZ-', 'BCD-', 'GHI-', 'VWX-', 'DEF-']:
        if bad in txt:
            fail(f'portal/dashboard.html  isolation breach → "{bad}"')
    if 'Oman Investment Bank' not in txt:
    ...
    for rel in ['erp/invoices.html', 'portal/dashboard.html', 'erp/sync.html']:
        load(rel)
        if 'OIB-SINV-2026-00841' not in pg.inner_text('body'):
            fail(f'{rel}  tracked invoice OIB-SINV-2026-00841 missing')
