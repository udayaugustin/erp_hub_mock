...
# Clone-drift: strings from the wj-mock, zubair-mock and prior-OIB-name bases
# that must never survive the rebrand to Oman Housing Bank. Scanned in visible
# HTML and in full JS. All tokens are case-sensitive substrings, so the
# uppercase 3-letter entity codes below cannot false-match lowercase words
# (OCS vs "docs" etc.). Bare 3-letter fragments are deliberately NOT banned
# (they collide with real English substrings, e.g. 'OSP' inside 'HOSPITALITY');
# multi-word / hyphen-suffixed forms are long enough to avoid that.
CLONE_DRIFT = ['Towell', 'WJ Towell', 'WJT-', 'ERPNext', 'Odoo',
               ...
               # zubair-mock brands and entity names/codes
               'Zubair', 'zubaircorp', 'ZUB-', 'Zakher', 'Al-Hilal', 'Dhofar',
               'Oasis Logistics', 'Sayarti', 'General Automotive',
               'SAP', 'S/4HANA', 'Fiori', 'Autoline', 'Orion', 'FOCUS X',
               'ZCL', 'AHI', 'OCS', 'ZED', 'ZBS', 'OLG', 'DAU', 'ZAG',
               'GAC', 'IHE', 'ZES', 'SAY',
               # prior "Oman Investment Bank (OIB)" naming for this mock
               'Oman Investment Bank', 'oibgroup', 'OIB Group', 'OIB Asset Management',
               'OIB Capital Markets', 'OIB Research & Advisory', 'OIB Trade Finance',
               'OIB Custody', 'OIB Salalah Payment', 'OIB Corporate Card',
               'OIB Digital Payments', 'OIB Treasury', 'OIB Coastal Payment',
               'OIB Merchant Settlement', 'CBO', 'Financial Services Authority',
               'OIB-', 'OAM-', 'OCM-', 'ORA-', 'OTF-', 'OCF-', 'OSP-', 'OCC-',
               'ODP-', 'OTR-', 'OSH-', 'OMS-']
...
with sync_playwright() as pw:
    ...
    # The single-entity portal shows Oman Housing Bank SAOC (OHB) only. No OTHER
    # group entity's name or document prefix may leak in. (Counterparty city
    # names like Salalah/Nizwa are legitimate — they are customers, not the
    # other group entities, so they are deliberately NOT banned here.)
    for bad in ['OHB Lending Services', 'OHB Deposit Services', 'OHB Engineering Services',
                'OHB Treasury & Financial Institutions', 'OHB Partnerships',
                'Iskan Digital Services', 'Integrated Cities', 'OHB Customer Experience',
                'OHB Mortgage Loan Servicing', 'OHB Salalah Branch', 'OHB Digital Transformation',
                'LEN-', 'DEP-', 'ENG-', 'TRE-', 'PTN-', 'ISK-', 'ICT-', 'CEX-', 'MLS-', 'SAL-', 'DGT-']:
        if bad in txt:
            fail(f'portal/dashboard.html  isolation breach → "{bad}"')
    if 'Oman Housing Bank' not in txt:
    ...
    for rel in ['erp/invoices.html', 'portal/dashboard.html', 'erp/sync.html']:
        load(rel)
        if 'OHB-SINV-2026-00841' not in pg.inner_text('body'):
            fail(f'{rel}  tracked invoice OHB-SINV-2026-00841 missing')
