# the OHB hub mock — gap register against the proposal
Re-pointed 17 Sep 2026 for the Oman Housing Bank VAT Group presentation. This build is a clone of
the proven multi-entity hub; the rules below are inherited from that build's audit and re-targeted
to the twelve-entity, single-VAT-group Oman Housing Bank story. `tools/verify.py` enforces them as
a lint over the user-visible markup and fails on any hit.
...
| # | Rule | Why |
|---|---|---|
| C1 | **One VAT Group, twelve legal entities.** All share VATIN `OM1200203817`. | The shared TRN is one filing identity; do not present it as twelve VAT registrations. |
...
| C4 | **Batch B2C.** OHB Customer Experience's ~66k/month simplified invoices are reported in **batches from High-Volume Billing**, not live at point of sale — carry the *"Assumption · to confirm with the bank"* card. | Load-bearing demo assumption. |
...
fictional buyer with an invented VAT number. Unlike the multi-entity mocks in this repo (Zubair,
Ras Al Hamra), the eleven business-unit "entities" here are **invented as separate VAT/legal
entities** — Oman Housing Bank has no subsidiaries. Six of the eleven do borrow real OHB
service-line/programme names (Lending Services, Deposit Services, Engineering Services, Treasury
& Financial Institutions, Partnerships, Iskan, Integrated Cities); the parent brand itself is
real. See `OHB-REAL-ENTITIES.md` for exactly which facts are real vs invented.
