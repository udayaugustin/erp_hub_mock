#!/usr/bin/env python3
"""Verify the Integrated Gas Company × Fawtara scope-walkthrough mock. Static sweep + live browser walk."""
import re, sys, pathlib, json
from playwright.sync_api import sync_playwright

ROOT = pathlib.Path(__file__).resolve().parents[1]   # igc-mock/
fails, warns = [], []
...
REAL_COMPANIES = ['OQ ', 'Oman LNG', 'OMIFCO', 'Petroleum Development Oman', 'Oman Oil Marketing', 'Sohar Aluminium', 'Muscat Municipality',
                  'Emirates Steel', 'Al Maha Petroleum', 'Renaissance Services',
                  'Bahwan Engineering']

# Clone-drift: strings from earlier mocks that must never survive in this one
# (single entity, generic ERP label — named ERP products, other clients and
# telecom vocabulary are all drift). Scanned in visible HTML and in full JS.
CLONE_DRIFT = ['Vodafone', 'vodafone', 'VOD-', 'prepaid', 'postpaid', 'top-up', 'Top-up',
               'SIM ', 'MSISDN', 'subscriber', 'bundle', 'Ooredoo', 'Omantel',
               'Oman Telecom', 'telecom', 'Telecom', 'telco', 'Consumer billing','Towell', 'WJ Towell', 'WJT-', 'wjtowell', 'ERPNext', 'Odoo',
...
# Named ERP products must not appear on the origin surface — IGC's ERP is
# not confirmed, so it is shown generically. (Checked live on the origin pages too.)
...
with sync_playwright() as pw:
    ...
    for rel in ['origin/billing.html', 'origin/nominations.html', 'origin/sync.html']:
        load(rel)
        ...
        if 'enterprise erp' not in txt.lower():
            fail(f'{rel}  does not use the generic "Enterprise ERP" label')
        if 'IGC' not in txt:
            fail(f'{rel}  does not name IGC')

    # --- the tracked invoice appears at the start, mid and end --------------
    for rel in ['origin/billing.html', 'hub/document.html', 'origin/sync.html']:
        load(rel)
        if 'IGC-INV-2026-00417' not in pg.inner_text('body'):
            fail(f'{rel}  tracked invoice IGC-INV-2026-00417 missing')

    # --- the OTA-position screens must carry the "to confirm" flag ----------
    for rel in ['hub/export.html', 'hub/takeorpay.html']:
        load(rel)
    ...
    # --- inbound covers producers and the transporter, landing as drafts ----
    load('hub/inbound.html')
    body = pg.inner_text('body').lower()
    for want in ['draft', 'producer', 'transport']:
        if want not in body:
            fail(f'hub/inbound.html  does not mention "{want}"')

    # --- adjustments screen shows spot, credit and debit notes --------------
    load('hub/adjustments.html')
    body = pg.inner_text('body').lower()
    for want in ['spot', 'credit note', 'debit note', 'igc-inv-2026-00390']:
        if want not in body:
            fail(f'hub/adjustments.html  does not mention "{want}"')

    # --- nominations screen shows metered volume and take-or-pay ------------
    load('origin/nominations.html')
    body = pg.inner_text('body').lower()
    for want in ['mmbtu', 'take-or-pay', 'nominat']:
        if want not in body:
            fail(f'origin/nominations.html  does not mention "{want}"')
