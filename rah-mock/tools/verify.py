...
# Clone-drift: strings from the wj-mock, zubair-mock and hub-mock (generic
# ABC Group) bases that must never survive the rebrand to Ras Al Hamra Group.
# Scanned in visible HTML and in full JS. Multi-word / prefixed tokens only —
# bare 3-letter fragments (DEF, RST, ...) are deliberately NOT banned here,
# since they collide with real English substrings (DEFAULT, FIRST); the
# uppercase entity-code bans below are long enough to avoid that.
CLONE_DRIFT = ['Towell', 'WJ Towell', 'WJT-', 'ERPNext', 'Odoo',
               ...
               'GAC-', 'IHE-', 'ZES-', 'SAY-',
               # hub-mock generic-placeholder base
               'The ABC Group', 'ABC Trading', 'XYZ Industries', 'BCD Services',
               'DEF Properties', 'GHI Building', 'JKL Logistics', 'MNO Automotive',
               'STU Motors', 'PQR Retail', 'YZA Heavy Equipment', 'VWX Enterprises',
               'RST Retail', 'abcgroup.example', 'GRP-0', 'High-Volume Billing',
               'ABC-', 'XYZ-', 'BCD-', 'DEF-', 'GHI-', 'JKL-', 'MNO-', 'STU-',
               'PQR-', 'YZA-', 'VWX-', 'RST-']
...
with sync_playwright() as pw:
    ...
    # The single-entity portal shows Ras Al Hamra LLC (RAH) only. No OTHER group
    # entity's name or document prefix may leak in. (Counterparty city names like
    # Salalah/Nizwa are legitimate — they are customers, not the other group
    # entities, so they are deliberately NOT banned here.)
    for bad in ['RAH Manpower Services', 'RAH Water Services', 'RAH Digital Transformation',
                'RAH Carbon Solutions', 'RAH Projects & Products', 'RAH Environmental Services',
                'RAH Equipment Trading', 'Middle East Consulting', 'Operation Excellence',
                'Add Energy & Partner', 'Innovative Oil and Gas',
                'MEC-', 'OPX-', 'ADE-', 'IOG-', 'RET-', 'RES-', 'RPP-', 'RMS-', 'RDT-', 'RCS-', 'RWS-']:
        if bad in txt:
            fail(f'portal/dashboard.html  isolation breach → "{bad}"')
    if 'Ras Al Hamra LLC' not in txt:
    ...
    for rel in ['erp/invoices.html', 'portal/dashboard.html', 'erp/sync.html']:
        load(rel)
        if 'RAH-SINV-2026-00841' not in pg.inner_text('body'):
            fail(f'{rel}  tracked invoice RAH-SINV-2026-00841 missing')
