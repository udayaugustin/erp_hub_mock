#!/usr/bin/env python3
"""Verify the ndc-mock against Proposal V3. Static sweep + live browser walk."""
...
ROOT = pathlib.Path(__file__).resolve().parents[1]   # ndc-mock/
fails, warns = [], []
...
BANNED_VISIBLE = [
    ...
    # clone drift: this mock was cloned from alnasr-mock (a marble company on
    # ERPNext). None of that vocabulary may survive in NDC's visible text.
    (r'\bAl Nasr\b', 'clone leftover: Al Nasr'),
    (r'\bMarbles?\b', 'clone leftover: marble'),
    (r'\bquarr(?:y|ies|ying)\b', 'clone leftover: quarry'),
    (r'\bERPNext\b', 'clone leftover: ERPNext — NDC runs Dynamics 365'),
    (r'AM Royal Beige', 'clone leftover: marble product'),
    (r'\bDarsait\b', 'clone leftover: Al Nasr location'),
...
NON_ENTITY_CODES = {'ERP', 'ASP', 'OTA', 'VAT', 'OMR', 'AED', 'USD',
                    ...
                    'CIV', 'VAN', 'INV', 'EXP', 'CRN'}   # doc-type / doc-number segments
DOC_SHAPE = re.compile(r"'([A-Z]{3}-(?:CIV|VAN|CRN|INV|SINV|CRNT)-\d{4}-\d+)'")
...
// Single company: TENANTS holds exactly one record (National Detergent Co) and
// GROUP is that company's own roll-up \u2014 no group, no hub/self-hosted split.
...
if HAVE_BROWSER:
  with sync_playwright() as pw:
    ...
    # --- single company on the portal --------------------------------------
    load('portal/dashboard.html')
    txt = pg.inner_text('body')
    # Nothing from the cloned builds may leak; the portal names only NDC.
    for bad in ['Al Nasr', 'Marbles', 'Towell', 'Terrazzo', 'ANM-', 'WJT-']:
        if bad in txt:
            fail(f'portal/dashboard.html  clone leftover → "{bad}"')
    if 'National Detergent' not in txt:
        fail('portal/dashboard.html  does not name its own company')
    ...
    for rel in ['erp/invoices.html', 'portal/dashboard.html', 'erp/sync.html']:
        load(rel)
        if 'NDC-CIV-2026-08841' not in pg.inner_text('body'):
            fail(f'{rel}  tracked invoice NDC-CIV-2026-08841 missing')
